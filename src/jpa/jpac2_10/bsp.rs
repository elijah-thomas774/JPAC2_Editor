...
use crate::ui_helpers::{color_edit, num_edit, nums_edit, write_color32};
...
impl BSP1TexScrollAnmData {
    pub fn show_editor(&mut self, ui: &mut Ui) {
        CollapsingHeader::new("Texture Animation Data").show(ui, |ui| {
            nums_edit(ui, &mut self.initial_trans, "Initial Translation: ");
            nums_edit(ui, &mut self.initial_scale, "Initial Scale: ");
            num_edit(ui, &mut self.initial_rot, "Initial Rotation: ");
            nums_edit(ui, &mut self.increment_trans, "Increment Translation: ");
            nums_edit(ui, &mut self.increment_scale, "Increment Scale: ");
            num_edit(ui, &mut self.increment_rot, "Increment Rotation: ");
        });
    }
}
...
impl FrameColor {
    fn show_editor(&mut self, ui: &mut Ui) -> Response {
        ui.horizontal(|ui| {
            num_edit(ui, &mut self.frame, "Frame:");
...
pub struct BSP1 {
    ...
    #[bw(calc = (0x34 // Base Size
        + if flags & 0x0100_0000 != 0 { 0x28 } else { 0x00 } // Texture Scroll Anim Data
    ...
    _size:                u32, // 0x04
    flags:                u32, // 0x08
    #[bw(calc = if color_flags & 0x02 != 0 { 0x34 + if flags & 0x0100_0000 != 0 {0x28} else {0} + ((tex_idx_anim_data.len() +3 ) & !3)} else {0} as u16)]
    _color_prm_offs:      u16, // 0x0C
    #[bw(calc = if color_flags & 0x08 != 0 {0x34 + if flags & 0x0100_0000 != 0 {0x28} else {0} + ((tex_idx_anim_data.len() + 3) & !3) + ((color_prm_data.len()*6 + 3) & !3)} else {0} as u16)]
